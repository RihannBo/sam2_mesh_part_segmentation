...
def normalize_modality(
    ...
    # Make sure mean/std live on the same device as imgs.
    mean = torch.tensor(img_mean, dtype=torch.float32, device=imgs.device).view(1, 3, 1, 1)
    std = torch.tensor(img_std, dtype=torch.float32, device=imgs.device).view(1, 3, 1, 1)
    imgs = (imgs - mean) / std
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state(
        ...
        """
        Initialize an inference state using in-memory tensors, while keeping
        the original SAM2 memory mechanism.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Load an original SAM2 video predictor from config + checkpoint and wrap
        it with the single-modality `SAM2VideoPredictor` interface used in this
        ablation (init from in-memory tensors, original memory).
        """
        from sam2.build_sam import build_sam2_video_predictor

        base = build_sam2_video_predictor(
