...
from seg3d.models.sam2_angular_mem_single_modality import SAM2AngularMemSingleModality



class SAM2AngularMemTrain(SAM2AngularMemSingleModality):
    def __init__(
        self,
        image_encoder,
        ...
        forward_backbone_per_frame_for_train=True,  # NEW
        ...
        **kwargs,
        ...
        self.forward_backbone_per_frame_for_train = forward_backbone_per_frame_for_train
        ...
        self.train_iou_heads = train_iou_heads
        self.rng = np.random.default_rng(seed=42)

        self._configure_trainable_params(
            train_iou_heads=train_iou_heads,
        )

    def forward(self, input: BatchedMeshDatapoint):
        forward_backbone_per_frame = (
            self.forward_backbone_per_frame_for_train
            if self.training
            else self.forward_backbone_per_frame_for_eval
        )
        if forward_backbone_per_frame:
            # Defer image feature computation on a frame until it's being tracked.
            # This helps avoid backbone OOM on long sequences / large batches.
            backbone_out = {"backbone_fpn": None, "vision_pos_enc": None}
        else:
            # Precompute image features on all frames before tracking.
            backbone_out = self.forward_image(
                {
                    "normal": input.flat_normal_batch,
                    "point": input.flat_point_batch,
                }
            )
    ...
    def _prepare_backbone_features_per_frame(self, normal_batch, point_batch, img_ids):
        """
        Compute the image backbone features on the fly for the given img_ids.
        Multi-modal version (normal + point).
        """
        ...
        normal = normal_batch[unique_img_ids]
        point = point_batch[unique_img_ids]
        ...
        return None, vision_feats, vision_pos_embeds, feat_sizes
    ...
    def forward_tracking(
        self, backbone_out, input: BatchedMeshDatapoint, return_dict=False
    ):
        """Forward video tracking on each frame (and sample correction clicks)."""

        # geometry_metas can be numpy array (view_dirs) or list (metas)
        if isinstance(input.geometry_metas, np.ndarray):
            assert input.geometry_metas.shape[0] == input.num_frames, \
                f"Expected {input.num_frames} view_dirs, got {input.geometry_metas.shape[0]}"
        else:
            assert len(input.geometry_metas) == input.num_frames, \
                f"Expected {input.num_frames} geometry metas, got {len(input.geometry_metas)}"
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                (
                    ...
                    input.flat_normal_batch, 
                    input.flat_point_batch, 
                    img_ids
                )
            ...
            current_out = self.track_step(
                ...
                view_dirs=input.geometry_metas,
            )
    ...
    def _iter_correct_pt_sampling(
        ...
        # Initialize sam_outputs with initial values in case the loop doesn't execute
        sam_outputs = (
            low_res_multimasks,
            high_res_multimasks,
            ious,
    ...
    def _configure_trainable_params(
        self,
        train_iou_heads: bool,
    ):
        """
        Authoritative training-scope configuration.

        Policy:
        - Freeze EVERYTHING by default (including patch embeddings)
        - Always train:
            - LoRA adapters
            - Fusion layers
        - Optionally train:
            - IoU head (if train_iou_heads=True)
        """
        ...
        # -------------------------------------------------
        # 2. Always train LoRA adapters
        # -------------------------------------------------
        for name, p in self.image_encoder.named_parameters():
            if any(k in name for k in ["A_q_", "B_q_", "A_v_", "B_v_"]):
                p.requires_grad = True

        # -------------------------------------------------
        # 3. Always train fusion layers
        # -------------------------------------------------
        for p in self.image_encoder.fusion.parameters():
            p.requires_grad = True

        # -------------------------------------------------
        # 4. Optionally train IoU head
        # -------------------------------------------------
        if train_iou_heads:
            for p in self.sam_mask_decoder.iou_token.parameters():
                p.requires_grad = True
            for p in self.sam_mask_decoder.iou_prediction_head.parameters():
                p.requires_grad = True
