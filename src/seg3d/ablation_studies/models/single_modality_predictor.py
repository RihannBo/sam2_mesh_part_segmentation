...
from seg3d.ablation_studies.models.sam2_angular_mem_single_modality import SAM2AngularMemSingleModality
...
def normalize_modality(
    ...
    """
    Normalize a single modality tensor in a way consistent with `load_video_frames`,
    with optional modality-specific behavior.

    Args:
        imgs: (T, 3, H, W) float tensor for a single modality
              (e.g. point AP render, normal map, matte, RGB).
        modality: String describing the modality; used to decide whether to
                  apply ImageNet mean/std after [0, 1] normalization.
        img_mean: ImageNet mean (default: (0.485, 0.456, 0.406)).
        img_std: ImageNet std  (default: (0.229, 0.224, 0.225)).

    Returns:
        imgs: (T, 3, H, W) normalized tensor.
    """
    ...
    imgs = _ensure_01_range(imgs)

    # For all current modalities (rgb / normal / point / matte),
    # use the same ImageNet-style normalization as `load_video_frames`.
    device = imgs.device
    mean = torch.tensor(img_mean, dtype=torch.float32, device=device).view(1, 3, 1, 1)
    std = torch.tensor(img_std, dtype=torch.float32, device=device).view(1, 3, 1, 1)
    imgs = (imgs - mean) / std
...
class SAM2AngularMemSingleModalityPredictor(SAM2AngularMemSingleModality):
    ...
    def init_state(
        ...
        view_dirs,
        modality: str = "generic",
        offload_inputs_to_cpu: bool = False,
        offload_state_to_cpu: bool = False,
    ):
        """
        Initialize an inference state from in-memory single-modality tensors.

        Args:
            images: (T, 3, H, W) float tensor for a single modality
                (e.g. point AP render, normal map, matte, RGB). Values can be
                in any range; they will be mapped to [0, 1] if needed and then
                ImageNet-normalized, matching `load_video_frames`.
            view_dirs: array-like of shape (T, 3) giving per-frame view
                directions; used for angular memory selection.
            modality: String describing the modality, e.g. "normal", "point",
                "matte", "rgb". This controls how `images` are normalized.
            offload_inputs_to_cpu: If True, keep `images` on CPU (moved to GPU lazily).
            offload_state_to_cpu: If True, offload inference state tensors to CPU
                to save GPU memory (at the cost of some runtime).
        """
        compute_device = self.device  # device of the model

        if images.dim() != 4 or images.size(1) != 3:
            raise ValueError(
                f"`images` must have shape (T, 3, H, W); got {tuple(images.shape)}"
            )

        num_frames, _, video_height, video_width = images.shape

        # Normalize the modality in a way consistent with `load_video_frames`
        # (auto map to [0, 1] if needed, then apply ImageNet mean/std).
        images = normalize_modality(images, modality=modality)
        ...
        # whether to offload the input frames to CPU memory
        ...
        inference_state["modality"] = modality
    ...
    def from_pretrained(
        ...
        **kwargs,
    ) -> "SAM2AngularMemSingleModalityPredictor":
        """
        Load a SAM2 model from config + checkpoint and wrap it with the
        single-modality angular-memory predictor used in this ablation.
        """
        from seg3d.ablation_studies.models.build_sam2 import build_multiview_sam2

        base = build_multiview_sam2(
            config_file=config_file,
            ckpt_path=ckpt_path,
            device=device,
        )
        base.__class__ = cls  # keep weights, swap to this interface

        # Ensure predictor-specific attributes exist even though we didn't run __init__
        base.fill_hole_area = kwargs.get(
            "fill_hole_area", getattr(base, "fill_hole_area", 0)
        )
        base.non_overlap_masks = kwargs.get(
            "non_overlap_masks", getattr(base, "non_overlap_masks", False)
        )
        base.clear_non_cond_mem_around_input = kwargs.get(
            "clear_non_cond_mem_around_input",
            getattr(base, "clear_non_cond_mem_around_input", False),
        )
        base.add_all_frames_to_correct_as_cond = kwargs.get(
            "add_all_frames_to_correct_as_cond",
            getattr(base, "add_all_frames_to_correct_as_cond", False),
        )
    ...
    # def _score_memories_by_overlap(self, frame_idx, output_dict, metas):
    #     """
    #     Score all available memories (cond + non-cond) by geometric overlap
    #     between metas[idx] and metas[frame_idx].

    #     Inputs:
    #         frame_idx    : current frame we are processing
    #         output_dict  : contains cond_frame_outputs and non_cond_frame_outputs
    #         metas        : list of per-view metadata dictionaries

    #     Returns:
    #         cond_scored    : list of (overlap, idx, out)
    #         noncond_scored : list of (overlap, idx, out)
    #     """ 
    #     if metas is None:
    #         raise RuntimeError(
    #             "_score_memories_by_overlap requires 'metas' from inference_state."
    #         )
    ...
    #     for idx, out in output_dict["cond_frame_outputs"].items():
    #         if idx == frame_idx:
    #             continue
    #         if out.get("maskmem_features", None) is None:
    #             continue
    #         overlap = compute_geometric_overlap(
    #             metas[idx],   # previous view meta
    #             curr_meta,    # current view meta
    #             sample_ratio=0.01,
    #             depth_tolerance=0.01,
    #         )
    #         cond_scored.append((overlap, idx, out))
        
    #     # 2) non-conditioning frames   
    #     for idx, out in output_dict["non_cond_frame_outputs"].items():
    #         if idx == frame_idx:
    #             continue
    #         if out.get("maskmem_features", None) is None:
    #             continue
    ...
    #             depth_tolerance=0.01,
    #         )
    #         noncond_scored.append((overlap, idx, out))
                
    #     # 3) sort by overlap descending
    #     cond_scored.sort(key=lambda x: x[0], reverse=True)
    #     noncond_scored.sort(key=lambda x: x[0], reverse=True)
        
    #     return cond_scored, noncond_scored
    
    # def _sort_memories_by_overlap(
    #     self,
    #     frame_idx,
    ...
    # ):
    #     #print('---- Function: sort memories by overlap ---')

    #     curr_meta = metas[frame_idx]
    #     scored = []

        
    #     for idx, out in unselected_cond_outputs.items():

    #         if out.get("maskmem_features", None) is None:
    #             continue

    #         # respect temporal direction
    #         if not track_in_reverse:
    #             # forward tracking → consider only frames BEFORE current
    #             if not (idx < frame_idx):
    #                 continue
    #         else:
    #             # reverse tracking → consider only frames AFTER current
    #             if not (idx > frame_idx):
    #                 continue
    ...
    #         scored.append((idx, out, overlap))
        
    #     for idx, out in non_cond_outputs.items():
    ...
    #     # for idx, _, overlap in scored:
            
    #     #     print(f"idx:{idx} --> overlap: {overlap}")
            
        
    #     return scored   # list of (idx, out, overlap)
    ...
    #     #print(f"\n=== Memory selection for frame {frame_idx} ===")
    ...
    #             # print(
    #             #     f" t_pos={t_pos:2d} | "
    #             #     f"orig_idx={prev_orig_idx if prev_orig_idx is not None else 'None'} | "
    #             #     f"overlap_idx={prev_overlap_idx}"
    #             # )
    ...
    #         # DEBUG
    #         #print(f"    orig_idx={orig_idx} -> overlap={overlap:.4f}, rank={rank} -> map to overlap_rank_idx={t_overlap_idx}")
    ...
    #     # Final Summary Print (Consolidated Debug)
    #     # print(f"[DEBUG: {frame_idx}] --- Cond Frame Selection Summary ---")
    #     # print(f"  Selected (orig_idx): {sorted(selected_outputs.keys())}")
    #     # print(f"  Unselected (orig_idx): {sorted(unselected_outputs.keys())}")
