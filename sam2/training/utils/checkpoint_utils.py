...
class CkptKeyMapperKernel:
    """
    Maps checkpoint keys from one pattern to another.
    Useful for loading checkpoints into models with different architectures
    (e.g., base encoder wrapped with LoRA).

    Args:
        key_mapping: Dictionary mapping old key prefixes to new key prefixes.
            Example: {"image_encoder.trunk": "image_encoder.lora.enc.base_model.model.trunk"}
    """
    ...
    def __call__(self, state_dict: Dict):
        ...
        for key, value in state_dict.items():
            new_key = key
            for old_prefix, new_prefix in self.key_mapping.items():
                if key.startswith(old_prefix + ".") or key == old_prefix:
                    new_key = key.replace(old_prefix, new_prefix, 1)
                    # Special handling for LoRA-wrapped modules: qkv.weight -> qkv.base_layer.weight
                    if ".attn.qkv.weight" in new_key:
                        new_key = new_key.replace(".attn.qkv.weight", ".attn.qkv.base_layer.weight")
                    elif ".attn.qkv.bias" in new_key:
                        new_key = new_key.replace(".attn.qkv.bias", ".attn.qkv.base_layer.bias")
                    break
...
def load_state_dict_into_model(
    ...
    """
    # Convert OmegaConf to plain dict if needed (PyTorch requires plain dict)
    from omegaconf import OmegaConf
    if OmegaConf.is_config(state_dict):
    ...
    if checkpoint_kernels is not None:
        for f in checkpoint_kernels:
            ...
            # Ensure kernels return plain dicts too
            if OmegaConf.is_config(state_dict):
                state_dict = OmegaConf.to_container(state_dict, resolve=True)
